...
- Name: API\petstore.spec.ts >> petstore api tests >> Create a new pet
- Location: tests\API\petstore.spec.ts:24:9
...
> 30 |         let respomsenbody=await response.json();
  ...
  38 |   const getresponse=await request.get('pet/78')
  39 | 
  40 |   let respomsenbody=await getresponse.json();
  41 |   console.log(respomsenbody); 
  42 |   expect(getresponse.status()).toBe(200);
  43 | })
  44 | })
```